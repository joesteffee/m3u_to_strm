...
def parse_series_name(tvg_name: str):
    ...
    # Require that group 3 has actual content (not just season/episode pattern)
    ...
    if duplicate_match2:
        ...
        # Validate that we captured actual show name content, not just season/episode
        # If it looks like just a season/episode pattern, fall through to normal parsing
        if show_name and not re.match(r'^S\d{1,2}\s*E\d{1,2}$', show_name, re.IGNORECASE):
